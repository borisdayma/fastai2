...
__all__ = ['ignore_exception', 'WandbCallback', 'wandb_process']
...
class WandbCallback(Callback):
    ...
    def _log_config(self):
        ...
        with ignore_exception(): config['batch size'] = self.dls.bs
        with ignore_exception(): config['batch per epoch'] = len(self.dls.train)
        with ignore_exception(): config['model parameters'] = total_params(self.model)[0]
        with ignore_exception(): config['loss function'] = f'{self.loss_func}'
        with ignore_exception(): config['device'] = self.dls.device.type
        with ignore_exception(): config['optimizer'] = self.opt_func.__name__
        with ignore_exception(): config['frozen'] = bool(self.opt.frozen_idx)
        with ignore_exception(): config['frozen idx'] = self.opt.frozen_idx
        with ignore_exception(): config['dataset.tfms'] = f'{self.dls.dataset.tfms}'
        with ignore_exception(): config['dls.after_item'] = f'{self.dls.after_item}'
        with ignore_exception(): config['dls.before_batch'] = f'{self.dls.before_batch}'
        with ignore_exception(): config['dls.after_batch'] = f'{self.dls.after_batch}'
